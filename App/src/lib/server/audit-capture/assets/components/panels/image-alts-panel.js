class ImageAltsPanel extends HTMLElement {
	constructor() {
		super();
	...
	render() {
      ...
      ${styles['image-alts-panel'] ? `<style>${styles['image-alts-panel']}</style>` : '<link rel="stylesheet" href="./styles/panels/image-alts-panel.css">'}
      <section class="section">
        <h1 class="title">${escapeHtml(panel.title ?? 'Unoptimized Alt Tags')}</h1>
      ...
      <section class="section">
        <div class="summary">
          ...
          <p class="summary-note">Missing image alt tags found on ${escapeHtml(panel.domain ?? 'this domain')}</p>
        </div>
      </section>
      <section class="section">
        <div class="list">
...
if (!customElements.get('image-alts-panel')) {
	customElements.define('image-alts-panel', ImageAltsPanel);
}
